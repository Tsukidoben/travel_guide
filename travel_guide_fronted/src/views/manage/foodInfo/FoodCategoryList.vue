<template>
  <div class="own-mvue-list-box">
    ...
    <!--表单-->
    <FoodCategoryForm ref="form" @reload="query(searchForm,pageBean)" :dialog-visible.sync="formDialog" :title.sync="formTitle" :action.sync="formAction" />
  </div>
</template>
...
import FoodCategoryForm from "@/views/manage/foodInfo/FoodCategoryForm.vue";
import SearchTop from "@/components/searchTop.vue";

export default {
  name: "FoodCategoryList",
  components: { FoodCategoryForm, SearchTop},
  mixins: [TableUtils],
  data() {
    return {
      delUrl: "/api/food/category/delete/",
      delBatchUrl: "/api/food/category/delBatch",
      searchList: [
        {type:'input',name:'关键字',placeholder:'分类名称',value:'keyword'},
      ]
    }
  },
  computed: {
  ...
  methods: {
    ...
    query(searchForm, pageBean) {
      request({
        url: config.backHost + "/api/food/category/listPage",
        method: 'POST',
        data: { 
